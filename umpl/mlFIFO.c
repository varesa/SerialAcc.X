...
#include "mltypes.h"		//  Added by Stephen Allen 8/11/11
...
inv_error_t inv_process_fifo_packet(const unsigned char *dmpData)
{
	...
	//MPL_LOGE("inv_process_fifo_packet decoded %d %d %d %d\n", fifo_obj.decoded[0],
	//												fifo_obj.decoded[1],
	//												fifo_obj.decoded[2],
	//												fifo_obj.decoded[3]);
...
inv_error_t inv_get_accel(long *data)
{
	...
	//MPL_LOGE("inv_get_accel data[0:2] %ld %ld %ld\n", data[0], data[1], data[2]);
...
inv_error_t inv_get_quaternion(long *data)
{
	...
	//MPL_LOGE("ref_quaternion data, %ld, %ld, %ld, %ld\n", fifo_obj.decoded[REF_QUATERNION],
...
inv_error_t inv_get_gyro(long *data)
{
    ...
    if (data == NULL)
		{
		MPL_LOGE("inv_get_gyro invalid parameter\n");
        return INV_ERROR_INVALID_PARAMETER;
		}
    ...
    if (fifo_obj.data_config[CONFIG_GYROS]) {
		...
		//MPL_LOGE("inv_get_gyro data[0:2] %ld %ld %ld\n", data[0], data[1], data[2]);
        return INV_SUCCESS;
    } else {
		//MPL_LOGE("inv_get_gyro feature not enabled\n");
        return INV_ERROR_FEATURE_NOT_ENABLED;
    }
