...
static struct prod_rev_map_t prod_rev_map[] = {
    ...
    /* line below added by Stephen Allen 9/22/11 */
	{MPL_PROD_KEY(4,  3), MPU_SILICON_REV_B1, 131,  8192}        /* (B3/F1)   */
};
		...
		struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	prod_rev >>= 2;
	/*  code below added by Stephen Allen 9/22/11 */
	if(prod_ver==4 && prod_rev==0)
    {
    	prod_rev=1; //Applicable for mantic revisions. when product version is 4
    }
