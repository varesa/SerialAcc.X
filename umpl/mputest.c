...
/******************************************************************************
 *
 * $Id: mputest.c 6179 2011-10-10 19:10:30Z mcaramello $
 *
 *****************************************************************************/
...
static int inv_populate_data_store(unsigned long sensor_mask, short temp_avg,
                                   short gyro_biases[], short accel_biases[],
                                   long accel_sens[]);
static int inv_test_accel(void *mlsl_handle, int enable_axes, short *bias,
                          long gravity, uint_fast8_t perform_full_test);
static int inv_test_gyro(void *mlsl_handle, short gyro_biases[3],
                         short *temp_avg, uint_fast8_t perform_full_test);
						 
						 
						 
//Port from Later version
...
static void usleep(unsigned long t)
{
    ...
    inv_get_tick_count(&start);
    do {
        inv_get_tick_count(&cur);
    } while ((cur - start) < (t / 1000));
#else
...
#ifdef UMPL
    if (accel_samples != test_setup.accel_samples) {
        MPL_LOGE("UMPL doesn't change the number of accel samples to avoid "
                 "dynamic memory allocation. Sorry!\n");
    }
#else
                  ...
                  uint_fast8_t perform_full_test)
{
...
#if UMPL_16BIT
    unsigned long test_start, test_end;
    ...
    for (j = 0; j < 3; j++) {
...
#else /* CONFIG_MPU_SENSORS_MPU6050B1 */
        result = inv_serial_single_write(
                    mlsl_handle, mldl_cfg->mpu_chip_info->addr,
        ...
        while (tmp-- > 0) {
            ...
            if (perform_full_test)
                usleep(DEF_PERIOD_CAL * 1000L);
            else
                usleep(DEF_PERIOD_SELF * 1000L);
    ...
    *temp_avg = (short)temperature;
    ...
    MPL_LOGI("\n");
#if defined UMPL_16BIT
    inv_get_tick_count(&test_end);
                            ...
                            short accel_biases[], long accel_sens[])
{
    ...
    result = inv_set_mpu_sensors(INV_SIX_AXIS_GYRO_ACCEL);
    if (result) {
        LOG_RESULT_LOCATION(result);
        return -1;
	}

    /* Only set biases for sensors requested in test. */
    if (sensor_mask & INV_THREE_AXIS_GYRO) {
        /* TODO: Incorrect scale. */
        for (ii = 0; ii < 3; ii++) {
            tmp[ii] = (long)((float)gyro_biases[ii] * 2 * test_setup.gyro_fs);
        }

        result = inv_set_gyro_bias_in_hw_unit(gyro_biases, INV_SGB_NO_MOTION);
        if (result) {
            LOG_RESULT_LOCATION(result);
            return -1;
		}        
    ...
    if (sensor_mask & INV_THREE_AXIS_ACCEL) {
        /* Get current accel bias first; not all axes may have been requested. */
        result = inv_get_accel_bias(tmp);
        if (result) {
            LOG_RESULT_LOCATION(result);
            return -1;
        }
        ...
        if (sensor_mask & INV_X_ACCEL)
            tmp[0] = accel_biases[0] * 65536L / accel_sens[0];
        if (sensor_mask & INV_Y_ACCEL)
            tmp[1] = accel_biases[1] * 65536L / accel_sens[1];
        if (sensor_mask & INV_Z_ACCEL)
            tmp[2] = accel_biases[2] * 65536L / accel_sens[2];

        result = inv_set_accel_bias(tmp);
        if (result) {
            LOG_RESULT_LOCATION(result);
            return -1;
        }
    ...
    /* Accel and gyro biases are already stored by the lite fusion ustore
     * delegate.
     */
...
gyro_test_failed:
accel_test_failed:
    /* Force the MPL to reset gyro settings. */
    ...
    if (saved_state < INV_STATE_DMP_STARTED) {
        ...
        if (result)
            return -1;
    } else if (inv_get_state() < INV_STATE_DMP_STARTED) {
        result = inv_dmp_start();
        if (result)
            return -1;
	}
